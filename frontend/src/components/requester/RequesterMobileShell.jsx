...
function MobileRequestForm({ form }) {
  ...
  return (
    <Card as="section" elevated className="run-requester-surface run-requester-surface--create requester-run-form-card requester-mobile-form-card">
      ...
      <details className="requester-mobile-funding" aria-label="Funding protection details"><summary><span><strong>Funding protection</strong><small>Secure hold preview</small></span><span className="requester-mobile-funding__amount">{formatMoney(preview.holdAmount)}</span><span className="requester-mobile-funding__view">View details</span></summary><div className="requester-run-form-preview" aria-live="polite"><div className="requester-run-form-preview__heading"><div><p className="requester-run-form-preview__eyebrow">Funding protection</p><h3>Secure hold preview</h3></div><span>Placeholder mode</span></div><div className="requester-run-form-preview__metrics"><div><p>Estimated hold</p><strong>{formatMoney(preview.holdAmount)}</strong></div><div><p>Max runner spend</p><strong>{formatMoney(preview.maxRunnerSpend)}</strong></div></div><p className="requester-run-form-preview__copy">Approved budget, payout, fee, and buffer protect requester funds and trigger receipt review when needed.</p></div></details>
    </Card>
  );
}
...
function MobileRunDetail({ run, onBack, onAuthorizeHold, onApproveManualReview, authorizingHold, approvingManualReview }) {
  if (!run) return null;
  const needsHold = Number(run.holdAmount || 0) > 0 && run.authorizationStatus !== "placeholder_authorized";
  ...
  return (
    <section className="requester-mobile-run-detail" aria-label="Run detail">
      ...
      {(showPrimaryHold || needsReview) && <div className="requester-mobile-detail-action">
        {showPrimaryHold && <><div><p className="requester-mobile-eyebrow">Funding authorization</p><strong>${run.holdAmount || 0} estimated hold</strong><span>Placeholder authorization only. No live charge is made.</span></div><Button fullWidth size="lg" variant="success" disabled={authorizingHold} onClick={() => onAuthorizeHold?.(run.id)}>{authorizingHold ? "Authorizing..." : "Authorize Secure Hold"}</Button></>}
      ...
      <details className="requester-mobile-disclosure">
        ...
        <div className="requester-mobile-disclosure__body">
          ...
          <p className="requester-mobile-disclosure__copy">{needsHold ? "Placeholder authorization only; no live charge is made." : "Spending protection supports receipt review when needed."}</p>
        </div>
      </details>
...
export default function RequesterMobileShell({
  ...
  const actionableHold = Boolean(
    activeRun && Number(activeRun.holdAmount || 0) > 0 && activeRun.authorizationStatus !== "placeholder_authorized"
  );
